...
;(function () {
  ...
  interface SentenceWithNormalized {
    original: string
    normalized: string
  }
  ...
  function updatePanelTexts(): void {
  ...
  function isValidCaptionContainer(container: Element): boolean {
  ...
  function findCaptionContainerByAriaLabel(ariaLabel: string): Element | null {
  ...
  async function clearSentenceHistory(): Promise<void> {
  ...
  async function initSentenceHistory(): Promise<void> {
    try {
  ...
  async function isSentenceProcessed(text: string): Promise<boolean> {
  ...
  async function markSentenceProcessed(text: string): Promise<void> {
  ...
  function startDrag(e: MouseEvent): void {
    ...
    isDragging = true
    dragOffsetX = e.clientX - (panelContainer as HTMLDivElement).offsetLeft
    dragOffsetY = e.clientY - (panelContainer as HTMLDivElement).offsetTop
  ...
  function onDrag(e: MouseEvent): void {
    ...
    const panelWidth = (panelContainer as HTMLDivElement).offsetWidth
    const panelHeight = (panelContainer as HTMLDivElement).offsetHeight
  ...
  function toggleMinimize(): void {
    ...
    if (isMinimized) {
      const currentRect = (panelContainer as HTMLDivElement).getBoundingClientRect()
      panelContainer!.style.bottom = 'auto'
      panelContainer!.style.right = 'auto'
  ...
  function addPendingItemToPanel(originalText: string): void {
    const emptyEl = document.getElementById('meet-translate-empty')
    if (emptyEl) emptyEl.remove()
  ...
  function updatePendingItemStatus(status: string): void {
  ...
  function finalizePendingItem(translatedText: string): void {
    if (pendingItemIndex === null) return
    const itemIndex = pendingItemIndex as number
  ...
  async function copyAllTranslations(): Promise<void> {
  ...
  function getBlockKey(textEl: Element): string {
    ...
    const allBlocks = captionContainer
      ? Array.from(captionContainer.querySelectorAll(CAPTION_SELECTORS.BLOCK))
      : []
  ...
  async function deduplicateBeforeSend(sentences: string[]): Promise<SentenceWithNormalized[]> {
    ...
    chrome.runtime.sendMessage(
      ...
      (response: unknown) => {
        ...
        if (resp && resp.success && resp.translatedText) {
          finalizePendingItem(resp.translatedText as string)
          ...
          console.warn('[Meet Translate] Translation failed:', resp?.error)
  ...
  function getSpeakerName(textEl: Element): string {
    const block = textEl.closest(CAPTION_SELECTORS.BLOCK)
    ...
    for (const textEl of textEls) {
      ...
      if (newSentences.length > 0) {
        ...
        if (activeSpeaker && currentSpeaker !== activeSpeaker) {
          console.log(
            `[Meet Translate] Speaker changed: "${activeSpeaker}" -> "${currentSpeaker}", flushing previous speaker's buffer`
          )
          flushBuffer()
        }
    ...
    chrome.storage.local.get([STORAGE_KEYS.API_KEY], (localItems: Record<string, unknown>) => {
      chrome.storage.sync.get(
        ...
        (syncItems: Record<string, unknown>) => {
          ...
          const extLang = (items[STORAGE_KEYS.EXTENSION_LANGUAGE] as string) || DEFAULT_LANGUAGE
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === 'sync') {
      if (changes[STORAGE_KEYS.SOURCE_LANGUAGE]) {
        sourceLanguage = changes[STORAGE_KEYS.SOURCE_LANGUAGE].newValue as string
      }
      if (changes[STORAGE_KEYS.TARGET_LANGUAGE]) {
        targetLanguage = changes[STORAGE_KEYS.TARGET_LANGUAGE].newValue as string
      }
      if (changes[STORAGE_KEYS.IS_ACTIVE]) {
        isActive = changes[STORAGE_KEYS.IS_ACTIVE].newValue as boolean
        if (!isActive) {
          updateStatus(t('panel.status_off'))
        }
      }
      if (changes[STORAGE_KEYS.EXTENSION_LANGUAGE]) {
        loadPanelTranslations(changes[STORAGE_KEYS.EXTENSION_LANGUAGE].newValue as string)
      }
    }
    if (areaName === 'local' && changes[STORAGE_KEYS.API_KEY]) {
      apiKey = changes[STORAGE_KEYS.API_KEY].newValue as string
    }
  })
