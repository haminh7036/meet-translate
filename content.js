...
(function () {
    ...
    function createPanel() {
        ...
        title.textContent = 'Meet AI Translator';
        ...
        emptyState.textContent = 'Đang chờ cuộc hội thoại...';
    ...
    function onDrag(e) {
        ...
        panelContainer.style.left = (e.clientX - dragOffsetX) + 'px';
        panelContainer.style.top = (e.clientY - dragOffsetY) + 'px';
    ...
    async function deduplicateBeforeSend(sentences) {
        ...
        for (const sentence of sentences) {
            ...
            if (seen.has(normalized)) {
                console.log(`[Meet Translate] Dedup (within buffer): "${normalized.substring(0, 40)}"`);
                continue;
            }

            const isProcessed = await isSentenceProcessed(normalized);
            if (isProcessed) {
                console.log(`[Meet Translate] Dedup (IndexedDB): "${normalized.substring(0, 40)}"`);
                continue;
            }
    ...
    async function flushBuffer() {
        ...
        const textToTranslate = deduplicated.join(' ');
        const sentenceCount = deduplicated.length;

        isTranslating = true;
        updateStatus(`Đang dịch ${sentenceCount} câu...`);
        ...
        chrome.runtime.sendMessage({
            ...
            if (response && response.success && response.translatedText) {
                addTranslationToPanel(textToTranslate, response.translatedText);
                updateStatus(`Đã dịch ${sentenceCount} câu`);

                deduplicated.forEach((s) => markSentenceProcessed(s));
            } else {
                console.warn('[Meet Translate] Translation failed:', response?.error);
                updateStatus('Dịch thất bại');
            }
        });
    }
